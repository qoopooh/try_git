...
for root, dirs, files in os.walk("./"):
    for name in dirs:
        lowname = name.lower()
        if "bin" in lowname or "obj" in lowname:
            svnrmdir(name, root)
devnull = open(os.devnull, 'w')
subprocess.Popen(['svn', 'ci', '-m', '"Remove generated folders"'], stdout=devnull) # still not work
...
for root, dirs, files in os.walk("./"):
    for name in files:
        ...
        if "conflicted" in name:
            rem(name=name, path=root)
devnull = open(os.devnull, 'w')
subprocess.Popen(['svn', 'ci', '-m', '"Remove generated files"'], stdout=devnull) # still not work
