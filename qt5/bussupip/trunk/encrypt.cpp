...
//-------------------------------------
void Encrypt(byte *msg, char phone_id)
{
  byte i,shift;
  ...
  while (msg[i-2] != LINE_FEED)
    ...
    if ((encr_decr_message[i] >= '0' && encr_decr_message[i] <= '9')
        || (encr_decr_message[i] >= 'A' && encr_decr_message[i] <= 'Z')
        || (encr_decr_message[i] >= 'a' && encr_decr_message[i] <= 'z'))
      {
      while (shift)
        {
...
//
//-------------------------------------
void Decrypt(byte *encrypted_msg)
{
  byte i,shift;
